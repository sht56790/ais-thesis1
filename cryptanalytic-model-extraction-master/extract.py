...
def _setup_result_logging():
  ...
  if len(sys.argv) > 1:
    args_slug = "_".join(sys.argv[1:])
  else:
    args_slug = "no_args"
  safe_slug = "".join(c if c.isalnum() or c in "-_." else "_" for c in args_slug)

  log_path = os.path.join(result_dir, f"extract_{timestamp}_{safe_slug}.log")
...
LOG_PATH = _setup_result_logging()
